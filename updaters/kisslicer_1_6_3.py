...
UPDATE_URL = "https://github.com/trilab3d/Slicer-profiles/archive/deltiq.zip"

slicer_path = os.path.dirname(os.path.realpath(__file__))

ref_path = os.path.join(slicer_path, "_reference")
bac_path = os.path.join(slicer_path, "_backup")
upd_path = os.path.join(slicer_path, "_update")
...
def perform_update_kisslicer_163():
    # Create update folder
    if os.path.isdir(upd_path):
        shutil.rmtree(upd_path) # remove if folder exists           
    os.makedirs(upd_path) # create new one
    
    upd_file_path = os.path.join(upd_path, 'deltiq.zip') # update zip file
    
    # Download update file from git
    urllib.request.urlretrieve(UPDATE_URL, upd_file_path)
    ...
    for file in zip_upd.namelist():
        if file.startswith('Slicer-profiles-deltiq/KISSlicer_1_6_3/'):
            zip_upd.extract(file, upd_path)
    zip_upd.close()
    
    # Create backup folder
    if os.path.isdir(bac_path):
        shutil.rmtree(bac_path) # remove if exists 
    os.makedirs(bac_path) # create new one
    ...
    # Check profiles
    for dir in PROFILE_DIRS: # loop for profiles folder
        dir_path = os.path.join(slicer_path, dir) # actual profile dir
        ref_dir_path = os.path.join(ref_path, dir) # reference profile dir
        upd_dir_path = os.path.join(upd_path, "Slicer-profiles-deltiq/KISSlicer_1_6_3/", dir) # update profile dir
    
        if os.path.exists(dir_path): # profile dir exists
            copydir(dir_path, os.path.join(bac_path, dir)) # create backup
    
            for file in os.listdir(dir_path): # loop for files inside profile dir
                file_path = os.path.join(dir_path, file) # current file
                ref_file_path = os.path.join(ref_path, dir, file) # reference file
        ...
        if os.path.exists(upd_dir_path): # Check update dir
            copydir(upd_dir_path, dir_path) # Update profile dir
    
            if os.path.exists(ref_dir_path): # if exists reference
                recursive_chmod(ref_dir_path, stat.S_IRWXU|stat.S_IRWXG|stat.S_IRWXO) # Set permission for delete
                shutil.rmtree(ref_dir_path) # remove old reference dir
            copydir(upd_dir_path, ref_dir_path) # copy new reference from update
        
    # Delete update dir
    shutil.rmtree(upd_path)
    
    # Set reference file for readonly
    recursive_chmod(ref_path, stat.S_IREAD|stat.S_IRGRP|stat.S_IROTH)
